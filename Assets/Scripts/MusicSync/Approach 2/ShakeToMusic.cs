...
public class ShakeToMusic : MonoBehaviour
{
    ...
    private void makeObjectsShakeScale()
    {
        if (bass)
        {
            float frequencyBass = MusicManager.instance.getFrequenciesDiapson(0, 7, 10) * factor;
            ScaleObject(transform, frequencyBass);
        }
        if (nb)
        {
            float frequencyNB = MusicManager.instance.getFrequenciesDiapson(7, 15, 100) * factor;
            ScaleObject(transform, frequencyNB);
        }
        if (middles)
        {
            float frequencyMiddles = MusicManager.instance.getFrequenciesDiapson(15, 30, 200) * factor;
            ScaleObject(transform, frequencyMiddles);
        }
        if (highs)
        {
            float frequencyHighs = MusicManager.instance.getFrequenciesDiapson(30, 32, 1000) * factor;
